...
This document describes recommended steps to deploy the College Entry System to managed platforms (Vercel for frontend) and Atlas for MongoDB.

## Overview
- Frontend: Next.js app — deploy to Vercel (recommended) or Netlify.
- Backend: Express app with Socket.IO — deploy as a Docker container to a Cloud VM (AWS EC2, DigitalOcean Droplet, etc.). Use Docker Hub or GHCR for images.
- Database: MongoDB Atlas — recommended for production.

## Prerequisites
- GitHub repository connected to your account
- Accounts: Vercel (frontend), Docker Hub or GitHub Packages (container registry)
...
2. Deploy to a Cloud VM (AWS EC2, DigitalOcean Droplet, etc.):
   - Install Docker on your VM
   - Pull the Docker image
   - Run the container with environment variables: `MONGODB_URI`, `JWT_SECRET`, `FRONTEND_ORIGIN`, `NODE_ENV=production`
3. Configure a reverse proxy (nginx) with HTTPS for WebSocket support.
...
- For Cloud VM, redeploy a previous Docker tag.
